...
import java.util.*;
import java.util.MyCollections;
...
public class MyHashtable<K, V> extends Dictionary<K, V>
     ...
     * {@link Map} interface in the MyCollections framework).
     *
    ...
    private void addEntry(int hash, K key, V value, int index) {
        ...
        tab[index] = new Entry<>(hash, key, value, e);// 有值的话，往前加，即当前的元素的next指向原值
        count++;
    }
    ...
    private <T> Enumeration<T> getEnumeration(int type) {
        if (count == 0) {
            return MyCollections.emptyEnumeration();
        } else {
    ...
    private <T> Iterator<T> getIterator(int type) {
        if (count == 0) {
            return MyCollections.emptyIterator();
        } else {
