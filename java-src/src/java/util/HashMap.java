...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        ...
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
        if ((p = tab[i = (n - 1) & hash]) == null)// 正常新增元素走这里
            tab[i] = newNode(hash, key, value, null);
        else {// i = (n - 1) & hash这个运算可能会得到两个相同的值，此时会走这里，在数组相同位置原元素的next指向新加入的元素
            Node<K,V> e; K k;
                ...
                for (int binCount = 0; ; ++binCount) {
                    if ((e = p.next) == null) {
                        ...
                        if (binCount >= TREEIFY_THRESHOLD - 1) // -1 for 1st
                            treeifyBin(tab, hash);// 如果数组中单个元素链表的长度大于等于8时，
                        break;
                    }
                    if (e.hash == hash &&
                        ((k = e.key) == key || (key != null && key.equals(k))))
    ...
    final Node<K,V>[] resize() {
        ...
        int newCap, newThr = 0;
        if (oldCap > 0) {// 正常扩容，数组容量和原threshold都扩为原来的2倍
        ...
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;
                    if (e.next == null)
                        newTab[e.hash & (newCap - 1)] = e;// 根据新的容量，重新计算数组下标
                        ...
                        if (loTail != null) {// 将低位数据放入新数组前半部分
                            loTail.next = null;
                            newTab[j] = loHead;
                        }
                        if (hiTail != null) {// 将高位数据放入新数组后半部分
                            hiTail.next = null;
     ...
     * 除非表太小，否则将替换给定哈希值的索引中bin中所有链接的节点，在这种情况下，将调整大小。
     */
     ...
     * linked node.
     * 进入树箱。 扩展LinkedHashMap.Entry（这又扩展了Node），
     * 因此可以用作常规或链接节点的扩展。
     */
            ...
            for (TreeNode<K,V> x = this, next; x != null; x = next) {
                ...
                if (root == null) {
                    ...
                    for (TreeNode<K,V> p = root;;) {
                        int dir, ph;
                        K pk = p.key;
                        if ((ph = p.hash) > h)// 比较新元素和根节点的hash值大小
                            dir = -1;// 放在左节点
                        else if (ph < h)
                            dir = 1;// 放在右节点
                        else if ((kc == null &&
                ...
                if (xp == (xppl = xpp.left)) {
                    if ((xppr = xpp.right) != null && xppr.red) {
                        ...
                        if (xp != null) {
                            ...
                            if (xpp != null) {
                                xpp.red = true;
                                root = rotateRight(root, xpp);// 树像左旋转
                            }
                        }
                    }
                }
